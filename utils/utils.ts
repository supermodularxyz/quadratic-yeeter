...
import { ApplicationMetadata, IPFSObject, InputType, MatchingStatsData, Program } from './types'
...
export const fetchFromIPFS = (cid: string) => {
  return fetch(`https://${process.env.NEXT_PUBLIC_PINATA_GATEWAY}/ipfs/${cid}`).then((resp) => {
    if (resp.ok) {
      return resp.json()
    }
 ...
 * @param cid - the unique content identifier that points to the data
 */
export const fetchFromGlobalGateway = (cid: string) => {
  return fetch(`https://${cid}.ipfs.dweb.link`).then((resp) => {
    if (resp.ok) {
      return resp.json()
    }
 ...
 */
export const getTxExplorerForContract = (chainId: ChainId, contractAddress: string) => {
  switch (chainId) {
    case ChainId.OPTIMISM_MAINNET_CHAIN_ID:
...
export const generateMerkleTree = (
  ...
  const tree = StandardMerkleTree.of(distribution, ['uint256', 'address', 'uint256', 'bytes32'])

  return { distribution, tree, matchingResults }
}

export const formatCurrency = (value: BigNumber, decimal: number, fraction?: number) => {
  return parseFloat(ethers.utils.formatUnits(value.toString(), decimal)).toLocaleString('en-US', {
